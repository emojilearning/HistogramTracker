...
void BresenhamCircle(Point2i veterx,int radius,vector<Point2i>& sampleset)
{
    ...
    {
        temp.emplace_back(veterx.x + p.x,veterx.y + p.y);
    }
    ...
    for (int i = 0; i < temp.size()-1; i++) {
        ...
        while(temp[i+1].y == temp[i].y)
            i++;
        auto y = temp[i].y;
        for (int x = left; x <= temp[i].x; ++x)
        {
            sampleset.emplace_back(x,y);
        }
    }
}

// get sample set in circle with given radius
void BresenhamCircle(int radius,vector<Point2i>& sampleset)
{
