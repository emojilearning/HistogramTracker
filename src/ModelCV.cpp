...
void Model::getContourPointsAndIts3DPoints(Sophus::SE3d &pose, std::vector<cv::Point3d> &verticesContour_Xs,
    ...
    for (int i = 0; i < visible_xs.cols; ++i) {
        cv::Point pt(visible_xs.at<double>(0, i), visible_xs.at<double>(1, i));
        //     LOG(INFO)<<i + 1<<" pt : "<<pt;
        if (pointInFrame(pt)){
            img1.at<int>(pt) = i + 1;
        }
...
//              img1.at<int>(contour[i].y+near[j][0],contour[i].x+near[j][1])=0;
                cv::Point3d pt3d( visible_Xs.at<double>(0, value - 1), visible_Xs.at<double>(1, value - 1),visible_Xs.at<double>(2, value - 1));
...
cv::Point Model::X_to_x(const cv::Point3f &X,const cv::Mat &extrisic)
{
  ...
  P.at<double>(2,0)=X.z;
  P.at<double>(3,0)=1;
  res=intrinsic_*extrisic*P;

  return cv::Point(res.at<double>(0,0)/res.at<double>(2,0),res.at<double>(1,0)/res.at<double>(2,0));
...
void Model::displayCV( Sophus::SE3d &pose,const cv::Scalar &color, cv::Mat& frame)
{
	...
	for (int i = 0; i < size; ++i) {
		cv::Point pt1(image_points.at<double>(0, 2*i), image_points.at<double>(1, 2*i));
		cv::Point pt2(image_points.at<double>(0, 2*i+1), image_points.at<double>(1, 2*i+1));
...
void Model::getVisualableVertices(Sophus::SE3d &pose, cv::Mat& vis_vertices) {
    cv::Mat pt_in_cam(3, VerticesCount()+1, CV_64F);
...
//    LOG(INFO)<<"extrinsic: "<<extrinsic;
    ...
    double u[3], v[3], n[3], c[3];
    for (size_t i = 0; i < model_->numtriangles; i++) {
        //compute the norm of the triangles
        u[0] = pt_in_cam.at<double>(0, model_->triangles[i].vindices[1]) - pt_in_cam.at<double>(0, model_->triangles[i].vindices[0]);
        u[1] = pt_in_cam.at<double>(1, model_->triangles[i].vindices[1]) - pt_in_cam.at<double>(1, model_->triangles[i].vindices[0]);
        u[2] = pt_in_cam.at<double>(2, model_->triangles[i].vindices[1]) - pt_in_cam.at<double>(2, model_->triangles[i].vindices[0]);

        v[0] = pt_in_cam.at<double>(0, model_->triangles[i].vindices[2]) - pt_in_cam.at<double>(0, model_->triangles[i].vindices[0]);
        v[1] = pt_in_cam.at<double>(1, model_->triangles[i].vindices[2]) - pt_in_cam.at<double>(1, model_->triangles[i].vindices[0]);
        v[2] = pt_in_cam.at<double>(2, model_->triangles[i].vindices[2]) - pt_in_cam.at<double>(2, model_->triangles[i].vindices[0]);

        Cross(u, v, n);
        Normalize(n);

        //center of triangle
        c[0] = 0.25f*pt_in_cam.at<double>(0, model_->triangles[i].vindices[0]) + 0.25f*pt_in_cam.at<double>(0, model_->triangles[i].vindices[1]) + 0.5f*pt_in_cam.at<double>(0, model_->triangles[i].vindices[2]);
        c[1] = 0.25f*pt_in_cam.at<double>(1, model_->triangles[i].vindices[0]) + 0.25f*pt_in_cam.at<double>(1, model_->triangles[i].vindices[1]) + 0.5f*pt_in_cam.at<double>(1, model_->triangles[i].vindices[2]);
        c[2] = 0.25f*pt_in_cam.at<double>(2, model_->triangles[i].vindices[0]) + 0.25f*pt_in_cam.at<double>(2, model_->triangles[i].vindices[1]) + 0.5f*pt_in_cam.at<double>(2, model_->triangles[i].vindices[2]);

        Normalize(c);

        //judge the whether the line is visible or not
        double cross = n[0] * c[0] + n[1] * c[1] + n[2] * c[2];
        if (cross < 0.0f) {
    ...
    for (size_t i = 0; i<model_->numLines; i++) {
        if ((model_->lines[i].e1 == 1 && model_->lines[i].e2 == 0) || (model_->lines[i].e1 == 0 && model_->lines[i].e2 == 1) || (model_->lines[i].e1 == 1 && model_->lines[i].e2 == 1)) {
            ...
            pos.at<double>(0, 2*vis_line_index) = model_->vertices[3 * v0];
            pos.at<double>(1, 2*vis_line_index) = model_->vertices[3 * v0 + 1];
            pos.at<double>(2, 2*vis_line_index) = model_->vertices[3 * v0 + 2];
            pos.at<double>(3, 2*vis_line_index) = 1;

            pos.at<double>(0, 2*vis_line_index+1) = model_->vertices[3 * v1];
            pos.at<double>(1, 2*vis_line_index+1) = model_->vertices[3 * v1 + 1];
            pos.at<double>(2, 2*vis_line_index+1) = model_->vertices[3 * v1 + 2];
...
void Model::project3D_2D( Sophus::SE3d &pose, const cv::Mat& visible_Xs,  cv::Mat &visible_xs)
{
    ...
    visible_xs=Mat(intrinsic34)*extrinsic*visible_Xs;
    for (int i = 0; i < visible_xs.cols; ++i) {
        double dz = 1.0f/visible_xs.at<double>(2, i);
        visible_xs.at<double>(0, i) *= dz;
        visible_xs.at<double>(1, i) *= dz;
    }
}
