...
//
//    NumericDiffCostFunctor(cv::Point2d &X, cv::Mat &fwd, cv::Mat &bg, cv::Mat &dt_map) :
...
//        auto rt = dt_map_.at<float>(cv::Point(xf,yc));
//        auto lt = dt_map_.at<float>(cv::Point(xc,yc));
//        auto mb = (lb + (x - xf)*(rb - lb));
//        auto mt = (lt + (x - xc)*(rt - lt));
//
//        auto dt = mb + (mt - mb)*(y - yf);
//
//        double Theta = (dt);
//        auto He = (1.0) / ((1) + ceres::exp(-Theta));
//        E = -ceres::log(He * fwd_.at<double>(xf) + (1 - He) * bg_.at<double>(yf));
//        residual[0] = E;
//        return true;
//    }
//
